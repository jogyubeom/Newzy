...
const Words = () => {
  ...
  const wordList = [
    {
      name : "공천",
      mean : ["1. 명사 여러 사람이 합의하여 추천함.", "2. 명사 공정하고 정당하게 추천함.", "3. 명사 행정 공인된 정당에서 선거에 출마한 당원을 공식적으로 추천하는 일."]
    },
    {
      name : "등용",
      mean : ["1. 명사 인재를 뽑아서 씀."]
    ...
    {
      name : "합의",
      mean : ["1. 명사 서로 의견이 일치함. 또는 그 의견.", "2. 명사 법률 둘 이상의 당사자의 의사가 일치함. 또는 그런 일."]
    },
  ]
  ...
  return (
    ...
    <div className="flex justify-end gap-10 items-center py-5 px-10">
      ...
      <button onClick={openModal} className="w-[234px] h-[60px] rounded-[45px] font-['Open_Sans'] bg-[#BF2EF0] hover:bg-[#A229CC] opacity-100 text-white text-[28px] font-semibold transition-colors duration-300">
        단어 테스트
      </button>
    </div>
    <div className="py-3 px-10 mx-auto font-sans mb-10">
      {wordList.map((word, index) => (
        <div key={index} className="mb-6 pb-4 border-b border-gray-300">
          ...
          <h2 className="text-blue-600 font-semibold mb-2 text-[24px]">{word.name}</h2>
          </div>
      ...
      {/* 모달 컴포넌트 렌더링 */}
      <WordTestModal isOpen={isModalOpen} onClose={closeModal} wordList={wordList} />
    </div>
    </>
  )
}
