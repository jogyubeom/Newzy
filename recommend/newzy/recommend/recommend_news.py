...
def recommend_news_by_cluster(cluster_id: int):
    ...
    if mid_user:
        ...
        for recommended_news in recommended_news_list:
            # 추천 결과 table 에 저장 (만약을 대비하여)
            save_recommended_news_by_cluster(cluster_id=cluster_id, news_id=recommended_news)
            # 추천 뉴스에 대한 요약 생성
            summarize_news(cluster_id=cluster_id, news_id=recommended_news)
...
def recommend_news():
    ...
    recommend_news_by_cluster(cluster_id=1)
    # for idx in range(1, cluster_cnt + 1):
    #     cluster_id = get_cluster_info(idx)  # pk 의 값이 id인 cluster가 있는지 조회
    #     if cluster_id:
    #         recommend_news_by_cluster(cluster_id=cluster_id)
    #     else:
    #         logging.error(f"cluster_id: {cluster_id} 와 일치하는 Cluster가 없어 건너뜁니다.")
    #         continue
